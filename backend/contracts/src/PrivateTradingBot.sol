...
contract PrivateTradingBot {
    ...
    function _withinDailyLimit() internal {
    ...
    function closePosition(
        ...
        if (currentPrice > entryPrice) {
            // Profit
            // casting to 'int256' is safe because (currentPrice - entryPrice) * amount / entryPrice will not exceed int256 max
            // forge-lint: disable-next-line(unsafe-typecast)
            pnl = int256((currentPrice - entryPrice) * amount / entryPrice);
        } else {
            // Loss
            // casting to 'int256' is safe because (entryPrice - currentPrice) * amount / entryPrice will not exceed int256 max
            // forge-lint: disable-next-line(unsafe-typecast)
            pnl = -int256((entryPrice - currentPrice) * amount / entryPrice);
        }
        
        // Return to balance (principal + profit/loss)
        // casting to 'int256' is safe because amount is uint256 and can be safely cast to int256
        // casting to 'uint256' is safe because the result will be non-negative (amount + pnl represents remaining funds)
        // forge-lint: disable-next-line(unsafe-typecast)
